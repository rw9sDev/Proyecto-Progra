...
def open_E2(parent: tk.Tk, return_to=None):
    win = tk.Toplevel(parent)
    win.title("Ejercicio 2")
    ...
    ruta = Path(__file__).resolve().parents[1] / "data" / "rutina2.csv"
    if not ruta.exists():
    ...
    # per-workout counter file for E2
    data_dir = ruta.parent
    count_file = data_dir / "e2_count.txt"
    def read_count():
        try:
    ...
    def save_and_return():
        ...
        if cur > 7:
            cur = 1
        write_count(cur)
        messagebox.showinfo("Guardado", f"Contador E2 guardado: {cur}")
        win.destroy()
        if return_to:
            return_to(parent)
